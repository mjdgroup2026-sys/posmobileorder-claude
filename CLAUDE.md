...
- 🔥 **SCB ไม่ยิง payment confirmation มาเลย 2 วันเต็ม โดยไม่มีสัญญาณผิดพลาดใด ๆ ให้จับ**
  ...
  · ตาข่ายรองรับที่มีตอนนี้คือ **การเตือนพนักงานเมื่อออก QR เกิน 3 นาทีแล้วยังไม่มี callback**
  (`listPaymentsAwaitingCallback()` ใน `lib/queries.ts`) — คำนวณสด ไม่ใช่แถวใน `Notification`
  จึงหายเองเมื่อ callback มาถึงหรือพนักงานปิดบิล และ **แค่เตือน ไม่ปิดบิลและไม่ยิงถามธนาคาร**
...
- **ชำระเงิน MJD Mobile Order — ✅ ต่อ SCB จริงแล้วและใช้งานได้ (พิสูจน์ด้วยเงินจริง 2026-09-09)**:
  ...
  · ตาข่ายรองรับคือ **เตือนพนักงานเมื่อออก QR เกิน 3 นาทีแล้วยังไม่มี callback** โผล่บนหน้า
