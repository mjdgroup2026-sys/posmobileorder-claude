...
# escape ข้อความให้เป็น JSON string ที่ถูกต้อง — jq ไม่ได้ติดตั้งไว้บนเครื่อง
#
# ⚠️ **host ไม่มี node** (node อยู่แต่ในคอนเทนเนอร์ของแอป) ทางที่ทำงานจริงบน VPS
#    จึงเป็น fallback ข้างล่างเสมอ — เวลาแก้ต้องทดสอบเส้นทางนั้นเป็นหลัก ไม่ใช่เส้นทาง node
#
# ⚠️ ของเดิมใช้ `s/$/\\n/` ซึ่ง sed มองว่า "ทุกบรรทัด" รวมบรรทัดสุดท้ายด้วย จึงเติม \n
#    ต่อท้ายเสมอแม้ข้อความมีบรรทัดเดียว · ผลคือ subject ของอีเมลมี \n ติดไปด้วย แล้ว
#    Resend ตอบ 422 "The \\n is not allowed in the \`subject\` field" — อีเมลแจ้งเตือน
#    ไม่เคยส่งออกเลยทั้งที่ทุกอย่างอื่นถูกหมด (เจอตอนทดสอบจริงบน production)
json_str() {
  if command -v node > /dev/null 2>&1; then
    node -e 'process.stdout.write(JSON.stringify(process.argv[1]))' "$1"
  else
    # escape \ กับ " ก่อน แล้วค่อยยุบขึ้นบรรทัดใหม่ "ที่อยู่ระหว่างบรรทัด" เป็น \n
    # (:a;N;$!ba คือรวมทุกบรรทัดเข้า pattern space ก่อน จึงแทนเฉพาะตัวคั่นจริง ๆ)
    printf '"%s"' "$(printf '%s' "$1" \
      | sed 's/\\/\\\\/g; s/"/\\"/g' \
      | sed ':a;N;$!ba;s/\n/\\n/g')"
  fi
